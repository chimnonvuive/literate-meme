...
vs = [vAs, vBs, vCs, vDs];
as = [aAs, aBs, aCs, aDs];
writematrix([real(rs), imag(rs)], 'system_positions');
writematrix([real(vs), imag(vs)], 'system_velocities');
writematrix([real(as), imag(as)], 'system_accelerations');
writematrix([alp2s, alp4s], 'system_angular_accs');
